...
mkdir -p "$WORK/marts" "$WORK/causal" "$WORK/models" "$WORK/bundles" "$WORK/logs"
...
echo "==> [4/4] package bundles"
run "$CAUSAL" python -m analysis.scripts.feature_mart.package_bundles \
    --models /work/models --marts /work/marts --out /work/bundles \
    2>&1 | tee "$WORK/logs/package.log"

echo "==> done: $WORK/bundles"
